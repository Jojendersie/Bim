...
bool BinaryModel::load(const char* _bimFile, const char* _matFile, Property::Val _requiredProperties, bool _loadAll)
{
	...
	m_dimScale = ei::IVec3(1, numChunks.x, numChunks.x * numChunks.y);
	m_requestedProps = _requiredProperties;
	m_loadedProps = _loadAll ? meta.propertyMask | _requiredProperties;
	m_boundingBox = meta.boundingBox;
	
	m_chunks.clear();
	Chunk emptyChunk;
	emptyChunk.properties = m_loadedProps;
	while(!m_file.eof())
	{
		m_file.read(reinterpret_cast<char*>(&header), sizeof(SectionHeader));
		if(header.type == CHUNK_SECTION)
		{
			...
			emptyChunk.address = m_file.tellg();
...
void BinaryModel::makeChunkResident(const ei::IVec3& _chunk)
{
	...
	{
		m_file.seekg(m_chunks[idx].address);
			...
			if(m_loadedProps & header.type != 0)
			{
				switch(header.type)
				{
					case Property::POSITION:
						...
						swap(m_chunks[idx].positions, std::vector<ei::Vec3>(header.size / sizeof(ei::Vec3)));
						m_file.read(reinterpret_cast<char*>(m_chunks[idx].positions.data()), header.size);
						break;
					case Property::NORMAL:
						swap(m_chunks[idx].normals, std::vector<ei::Vec3>(header.size / sizeof(ei::Vec3)));
						m_file.read(reinterpret_cast<char*>(m_chunks[idx].normals.data()), header.size);
						break;
					default: file.seekg(header.size, std::ios_base::cur);
				}
			} else file.seekg(header.size, std::ios_base::cur);
			m_file.read(reinterpret_cast<char*>(&header), sizeof(SectionHeader));
		}
...
void BinaryModel::storeChunk(const char* _bimFile, const ei::IVec3& _chunkPos)
{
	...
	header.size = m_chunks[idx].positions.size() * sizeof(ei::Vec3) +
				  m_chunks[idx].normals.size() * sizeof(ei::Vec3);
	file.write(reinterpret_cast<char*>(&header), sizeof(SectionHeader));
	...
	if(m_loadedProps & Property::POSITION)
	{
		...
		header.size = m_chunks[idx].positions.size() * sizeof(ei::Vec3);
		file.write(reinterpret_cast<char*>(&header), sizeof(SectionHeader));
		file.write(reinterpret_cast<char*>(m_chunks[idx].positions.data()), header.size);
	}
	if(m_loadedProps & Property::NORMAL)
	{
		header.type = Property::NORMAL;
		header.size = m_chunks[idx].normals.size() * sizeof(ei::Vec3);
		file.write(reinterpret_cast<char*>(&header), sizeof(SectionHeader));
		file.write(reinterpret_cast<char*>(m_chunks[idx].normals.data()), header.size);
	}
}